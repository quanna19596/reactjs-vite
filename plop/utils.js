...
export const getColorName = {
  init: function() {
    var color, rgb, hsl;
    for(var i = 0; i < getColorName.names.length; i++)
    {
      color = "#" + getColorName.names[i][0];
      rgb = getColorName.rgb(color);
      hsl = getColorName.hsl(color);
      getColorName.names[i].push(rgb[0], rgb[1], rgb[2], hsl[0], hsl[1], hsl[2]);
    }
  },
  ...
  name: function(color) {
    ...
    var rgb = getColorName.rgb(color);
    var r = rgb[0], g = rgb[1], b = rgb[2];
    var hsl = getColorName.hsl(color);
    ...
    for(var i = 0; i < getColorName.names.length; i++)
    {
      if(color == "#" + getColorName.names[i][0])
        return ["#" + getColorName.names[i][0], getColorName.names[i][1], true];

      ndf1 = Math.pow(r - getColorName.names[i][2], 2) + Math.pow(g - getColorName.names[i][3], 2) + Math.pow(b - getColorName.names[i][4], 2);
      ndf2 = Math.pow(h - getColorName.names[i][5], 2) + Math.pow(s - getColorName.names[i][6], 2) + Math.pow(l - getColorName.names[i][7], 2);
      ndf = ndf1 + ndf2 * 2;
      if(df < 0 || df > ndf)
      {
        df = ndf;
    ...
    return (cl < 0 ? ["#000000", "Invalid Color: " + color, false] : ["#" + getColorName.names[cl][0], getColorName.names[cl][1], false]);
  },
